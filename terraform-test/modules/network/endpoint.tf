...
resource "aws_vpc_endpoint" "s3_endpoint" {
  ...
  route_table_ids = concat(
    [
      for rt in aws_route_table.std19_private_rt : rt.id
    ],
    [
      aws_route_table.std19_cluster_rt.id
    ]
  )
...
resource "aws_vpc_endpoint" "ecr_api" {
  ...
  subnet_ids  = toset([
    for subnet in aws_subnet.std19_private_subnet : subnet.id
  ])

  # ECR은 통신포트로 443을 사용
  security_group_ids  = [
    aws_security_group.std19_eks_sg.id
  ]
